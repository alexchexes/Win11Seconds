...
- Native minimize/enlarge/close window controls? We added a custom cross for a reason and removed the default Windows top window bar to make the whole window a monolithic surface without differentiated areas; we should not break that. Maybe we don't need that.
